  ...
  (save-excursion
    (let ((continue (pga--previous-heading-same-level))
      ...
      (while (and (not result) continue)
        (when (and (not (pjs-gtd-project-p)) (org-get-todo-state) (pjs-gtd-action-p))
          (setq result t))
        (setq continue (pga--previous-heading-same-level)))
      result)))

(defun pga--next-action-p ()
  (and (not (pjs-gtd-project-p))
       (org-get-todo-state)
       (pjs-gtd-action-p)))
...
;;  'pjs-gtd-agenda
;;  )
;; (provide-theme 'pjs-gtd-agenda)
;; (enable-theme 'pjs-gtd-agenda)

(provide 'pjs-gtd-agenda)
;;; pjs-gtd-agenda.el ends here

;; Local Variables:
;; read-symbol-shorthands: (("pga-" . "pjs-gtd-agenda-"))
;; End:
